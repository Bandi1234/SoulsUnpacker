...
namespace SoulsUnpackTools {
    public static class SEKTools {
        ...
        public static void RepackPureText(string itemFolder, string menuFolder, string itemTarget, string menuTarget, CommonUtils.TextObserver observer) {
            ...
            StreamReader fmgInfoReader = new StreamReader(Path.Combine(itemFolder, "compressionIds.SEKInfo"));
            ...
            foreach (FmgInfo info in itemInfos) {
                ...
                fmg.Version = FMG.FMGVersion.DarkSouls3;
                fmg.Compression = DCX.Type.None;
                using (StreamReader reader = new StreamReader(Path.Combine(folder, "empty.SEKInfo"))) {
                ...
                itemEntries++;
                observer.onItemProgress(itemEntries, maxItemEntries);
                using (StreamReader reader = new StreamReader(Path.Combine(folder, "spaces.SEKInfo"))) {
                    while (!reader.EndOfStream) {
                        string line = reader.ReadLine();
                        if (line != "") {
                            FMG.Entry entry = new FMG.Entry(int.Parse(line), " ");
                ...
                BinderFile bFile = new BinderFile();
                bFile.Name = @"N:\NTC\data\Target\INTERROOT_win64\msg\engUS\" + DS3Utils.GetItemJapaneseFmg(info.name) + ".fmg";
                bFile.ID = info.id;
                bFile.CompressionType = DCX.Type.Zlib;
                bFile.Bytes = fmg.Write();
                item.Files.Add(bFile);
            ...
            fmgInfoReader = new StreamReader(Path.Combine(menuFolder, "compressionIds.SEKInfo"));
            ...
            foreach (FmgInfo info in menuInfos) {
                ...
                BinderFile bFile = new BinderFile();
                bFile.Name = @"N:\NTC\data\Target\INTERROOT_win64\msg\engUS\" + DS3Utils.GetMenuJapaneseFmg(info.name) + ".fmg";
                bFile.ID = info.id;
                bFile.CompressionType = DCX.Type.Zlib;
                bFile.Bytes = fmg.Write();
                menu.Files.Add(bFile);
        ...
        public static void UnpackSEKText(string itemSource, string menuSource, string itemTarget, string menuTarget, CommonUtils.TextObserver observer) {
        ...
        public static void RepackSEKText(string itemSource, string menuSource, string itemTarget, string menuTarget, CommonUtils.TextObserver observer) {
